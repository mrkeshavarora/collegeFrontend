...
const About = () => {
  ...
  return (
    <div className="bg-gray-50 pb-20">
      <SmallBlueStrap head={`About ${collegeData.name}`} line="Building a legacy of excellence and innovation in education." />
          ...
          <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
            ...
            <div className="w-full lg:w-1/2 space-y-10">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="flex-1 bg-blue-50/50 p-6 rounded-2xl border border-blue-100 flex flex-col gap-3 transition-all hover:shadow-md hover:bg-white">
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">{collegeData.mission}</p>
                </div>
                <div className="flex-1 bg-blue-50/50 p-6 rounded-2xl border border-blue-100 flex flex-col gap-3 transition-all hover:shadow-md hover:bg-white">
                  <FaEye className='bg-[#155DFC] text-white p-2 rounded-xl' size={"42px"} />
                  <h3 className='text-xl font-bold text-[#1C398E]'>Our Vision</h3>
                  <p className="text-gray-600 text-sm leading-relaxed">{collegeData.vision}</p>
                </div>
              </div>
            </div>
            ...
            <div className="w-full lg:w-1/2">
              <div className="relative rounded-3xl overflow-hidden shadow-xl">
                <img
                  src={
                    Array.isArray(collegeData.image) && collegeData.image.length > 0 
                      ? (collegeData.image[0].startsWith('http') ? collegeData.image[0] : `${import.meta.env.VITE_API_URL}/${collegeData.image[0]}`)
                      : (collegeData.image && collegeData.image.startsWith('http') ? collegeData.image : collegeData.image ? `${import.meta.env.VITE_API_URL}/${collegeData.image}` : "")
                  }
          ...
          <div className="mt-16 pt-10 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-gray-50/50">
              ...
              <div className="space-y-1">
                ...
                <div className="text-gray-600 text-sm">
                  <p className="font-medium flex items-center gap-1"><FaPhone size={12} /> {collegeData.phone}</p>
                  <p className="flex items-center gap-1"><FaEnvelope size={12} /> {collegeData.email}</p>
                </div>
              </div>
            </div>
